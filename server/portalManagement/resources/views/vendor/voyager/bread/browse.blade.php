                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'checkbox')
                                                    @if(property_exists($row->details, 'on') && property_exists($row->details, 'off'))
                                                        @if($data->{$row->field})
                                                            ...
                                                            <?php $company = \App\Models\Company::find($dataTypeContent->{$row->field}); ?>
                                                        @else
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    @if($row->field == 'sub_company_id')
                                                        @if($data->{$row->field})
                                                            <?php $subCompany = \App\Models\SubCompany::find($data->{$row->field}); ?>
                                                            <a href="{{ route('voyager.sub-companies.show', $data->{$row->field}) }}">{{ $subCompany->name }}</a>
                                                        @else
                                                            <span>Unemployed</span>
                                                        @endif
                                                    @elseif($row->field == 'company_id')
                                                        @if($data->{$row->field})
                                                            <?php $company = \App\Models\Company::find($data->{$row->field}); ?>
                                                            <a href="{{ route('voyager.companies.show', $data->{$row->field}) }}">{{ $company->name }}</a>
                                                        @else
                                                            <span>Unemployed</span>
                                                        @endif
